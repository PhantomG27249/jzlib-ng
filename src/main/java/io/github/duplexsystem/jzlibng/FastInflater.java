...
public class FastInflater {
     ...
     * @see FastInflater#needsDictionary
     * @see FastInflater#getAdler
     */
    ...
    public void setDictionary(ByteBuffer dictionary) {
        synchronized (zsRef) {
            ...
            if (dictionary.isDirect()) {
                long address = UnsafeUtils.getFieldLong(dictionary, "address");
                try {
     ...
     * @see FastInflater#setDictionary
     */
            ...
            try {
                if (input == null) {
                    ...
                    try {
                        ...
                        if (input.isDirect()) {
                            try {
                                long inputAddress = UnsafeUtils.getFieldLong(input, "address");
        ...
        synchronized (zsRef) {
            ...
            try {
                if (input == null) {
                    ...
                    try {
                        if (output.isDirect()) {
                            long outputAddress = UnsafeUtils.getFieldLong(output, "address");
                            try {
    ...
    static class InflaterZStreamRef implements Runnable {
        ...
        private InflaterZStreamRef(FastInflater owner, long addr) {
            this.cleanable = (owner != null) ? ((Cleaner)UnsafeUtils.getStaticFieldObject(ReflectionUtils.getClass("jdk.internal.ref.CleanerFactory"), "commonCleaner")).register(owner, this) : null;
            this.address = addr;
        }
