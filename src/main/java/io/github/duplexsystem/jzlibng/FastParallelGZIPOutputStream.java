...
public class FastParallelGZIPOutputStream extends FastGZIPOutputStream {
    ...
    private final UnfairExecutor unfairExecutor = new UnfairExecutor(executorThreadCount);
    private final FastCRC32 fastCRC = new FastCRC32();
    private final int emitQueueSize;
    private final BlockingQueue<Future<Block>> emitQueue;

    private static FastDeflater newDeflater() {
        return new FastDeflater(FastDeflater.DEFAULT_COMPRESSION, true);
    }
    
    private static FastDeflaterOutputStream newDeflaterOutputStream(OutputStream out, FastDeflater fastDeflater) {
        return new FastDeflaterOutputStream(out, fastDeflater, 512, true);
    ...
    private static class State {
        ...
        private final FastDeflater def = newDeflater();
        private final ByteArrayOutputStreamExposed buf = new ByteArrayOutputStreamExposed(SIZE + (SIZE >> 3));
        private final FastDeflaterOutputStream str = newDeflaterOutputStream(buf, def);
    }
    ...
    private static int getThreadCount(UnfairExecutor executor) {;
        //ececutorThreadCount is hard coded to system thread count in this case so just returning that
  ...
  //  private final ExecutorService executor;
    ...
    public FastParallelGZIPOutputStream(OutputStream out, UnfairExecutor executor, int nthreads) throws IOException {
        super(out);
        executor = this.unfairExecutor;
        nthreads = this.executorThreadCount;
    ...
    /*
    *     public FastParallelGZIPOutputStream(OutputStream out, ExecutorService executor, int nthreads) throws IOException {
        super(out);
    ...
    * */
    ...
    @Deprecated // Doesn't really use the given number of threads.
    public FastParallelGZIPOutputStream(OutputStream out, int nthreads) throws IOException {
        this(out, FastParallelGZIPEnviroment.getSharedThreadPool(), nthreads);
    }

    public FastParallelGZIPOutputStream(OutputStream out, UnfairExecutor executor) throws IOException {
        this(out, executor, getThreadCount(executor));
    }

    /**
     * Creates a ParallelGZIPOutputStream
     * using {@link FastParallelGZIPEnviroment#getSharedThreadPool()}.
     *
     * @param out the eventual output stream for the compressed data.
     * @throws IOException if it all goes wrong.
     */
    public FastParallelGZIPOutputStream(OutputStream out) throws IOException {
        this(out, FastParallelGZIPEnviroment.getSharedThreadPool());
    }
    ...
    private void submit() throws IOException {
        ...
        emitQueue.add(unfairExecutor.submit(block));
        Block b = freeBlock;
        if (b != null)
            freeBlock = null;
        else
    ...
    public void close() throws IOException {
        ...
        //TO DO replace bytebuffer with unsafe
        if (bytesWritten >= 0) {
            flush();
            ...
            buf.putInt((int) fastCRC.getValue());
